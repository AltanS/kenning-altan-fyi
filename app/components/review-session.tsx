...
export function ReviewSessionView({ cards, heading, backTo, backLabel }: ReviewSessionViewProps) {
  ...
  if (isReviewComplete(session) || card === null) {
    return (
      <div className="mx-auto flex w-full max-w-md flex-col gap-4 text-center">
        ...
        <Link to={backTo} className="text-sm text-brand-ink underline underline-offset-4">
          {backLabel}
        </Link>
      </div>
    );
  ...
  return (
    <div className="mx-auto flex w-full max-w-md flex-col gap-4">
      ...
      />
      <Link to={backTo} className="text-center text-sm text-brand-ink underline underline-offset-4">
        {backLabel}
      </Link>
    </div>
  );
